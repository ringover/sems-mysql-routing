    ...
    PayloadIdMapping &transcoder_payload_mapping)
{
  ...
  DBG("filtering SDP for request '%s'\n",
      req.method.c_str());
  if (body && (req.method == SIP_METH_MESSAGE || req.method == SIP_METH_INVITE || req.method == SIP_METH_UPDATE || req.method == SIP_METH_ACK)) {
    ...
    PayloadIdMapping &transcoder_payload_mapping)
{
  ...
  if (body && (reply.cseq_method == SIP_METH_INVITE || reply.cseq_method == SIP_METH_UPDATE || reply.cseq_method == SIP_METH_MESSAGE )) {
    filterBody(body, sdp, call_profile, a_leg, transcoder_payload_mapping);
  }
}
...
//  throw AmSession::Exception(488,"Not accepted here");
  DBG("SIP MESSAGE ==> SBCFacory::onMessage");
...
void SBCDialog::onMessage(const AmSipRequest& req){
  AmUriParser ruri_parser, from_parser, to_parser;
...
{
  DBG("=====================  SBCDialog::process event_id=%d",ev->event_id);
...
void SBCCalleeSession::process(AmEvent* ev) {
  SBCControlEvent* ctl_event;
  DBG("=====================  SBCCalleeSession::process event_id=%d",ev->event_id);
  if (ev->event_id == SBCControlEvent_ID &&
      (ctl_event = dynamic_cast<SBCControlEvent*>(ev)) != NULL) {
    onControlCmd(ctl_event->cmd, ctl_event->params);
    return;
...
{
  SBCRelayController rc(&call_profile.transcoder, false);
  if (call_profile.transcoder.isActive()) {
    if (media_session) return media_session->updateRemoteSdp(a_leg, sdp, &rc);
  }
