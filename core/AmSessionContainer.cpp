					     ...
					     AmArg* session_params)
{
  ...
  if (req.method == "INVITE") {
    ...
    if (NULL != session_params) {
      session = session_factory->onMessage(req, app_name, *session_params);
    }
    else {
      session = session_factory->onMessage(req, app_name, app_params);
    }
  }
  ...
  if(!session) {
    ...
    DBG("onInvite/onMessage/onRefer returned NULL\n");
  }
  else {
