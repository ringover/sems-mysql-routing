...
int SessionTimer::configure(AmConfigReader& conf)
{
  ...
  if (conf.hasParameter("session_refresh_method")) {
    ...
    //} else if (refresh_method_s == "UPDATE_FALLBACK_MESSAGE") {
    //  s->refresh_method = AmSession::REFRESH_UPDATE_FB_REMES;
    } else if (refresh_method_s == "INVITE") {
      s->refresh_method = AmSession::REFRESH_REINVITE;
    //} else if (refresh_method_s == "MESSAGE") {
    //  s->refresh_method = AmSession::REFRESH_REMESSAGE;
    } else {
