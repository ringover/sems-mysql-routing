...
AmSession* AmSessionFactory::onMessage(const AmSipRequest& req, const string& app_name,
                                      AmArg& session_params) {
  WARN(" => MESSAGE : discarding session parameters to new session.\n");
  map<string,string> app_params;
  return onMessage(req,app_name,app_params);
}

AmSession* AmSessionFactory::onMessage(const AmSipRequest& req, const string& app_name, const map<string,string>& app_params)
{
  throw AmSession::Exception(488,"SIP Message Not accepted here");
}

AmSession* AmSessionFactory::onInvite(const AmSipRequest& req, const string& app_name,
				      AmArg& session_params) {
  WARN(" => Invite : discarding session parameters to new session.\n");
  map<string,string> app_params;
